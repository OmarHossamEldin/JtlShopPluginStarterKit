const createCategory = (form,loading, popUpContainer,popUpContent,popUpIcon,popUpMessage,popUpConfirm,result, prev, next) => {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const response = await fiber.post("/admin/io.php/categories", data);
    ...
    if (response.status === 422) {
      ...
      if (result.childElementCount < result.dataset.resultPerPage) {
        drawCategoriesTable(result, prev, next, loading, response, null, category);
      } else {
