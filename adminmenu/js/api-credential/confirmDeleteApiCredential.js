const confirmDeleteApiCredential = (result, loading, popUpContainer, popUpContent, popUpIcon, popUpMessage, popUpConfirm) => {
 // const baseUrl = `${location.protocol}//${location.host}/shop`;
  ...
  popUpConfirm.onclick =  async () => {
    ...
    const response = await fiber.post(`/admin/io.php/delete/api-credentials/${id}`, data);
    loading.style.display = "none";
    if (response === 204) {
      popUpIcon.src = `${baseUrl}/mediafiles/Resources/images/accept.png`;
      const paragraph = document.createElement("p");
      paragraph.innerHTML = "Api Credential Deleted Successfully";
      ...
      const paragraph = document.createElement("p");
      paragraph.innerHTML =
        "Please Make Sure Selected api-credentials!!";
      popUpMessage.innerHTML = paragraph.innerHTML;
    }
