const createEmailCredentials = (form,loading, popUpContainer,popUpContent,popUpIcon,popUpMessage,popUpConfirm,result, prev, next) => {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const response = await fiber.post("/admin/io.php/email-credentials", data);
    ...
    if (response.status === 422) {
      ...
      popUpMessage.appendChild(paragraph);
      const emailCredentials = { ...response.data.emailCredentials };
      if (result.childElementCount < result.dataset.resultPerPage) {
        drawCategoriesTable(result, prev, next, loading, response, null, emailCredentials);
      } else {
  ...
  const popUpClose = document.querySelector(".dismiss-popup.email-credential");
