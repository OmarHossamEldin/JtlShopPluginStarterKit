const createPost = (form,loading, popUpContainer,popUpContent,popUpIcon,popUpMessage,popUpConfirm,result, prev, next) => {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const response = await fiber.post("/admin/io.php/posts", data);
    ...
    if (response.status === 422) {
      ...
      if (result.childElementCount < result.dataset.resultPerPage) {
        drawPostsTable(result, prev, next, loading, response, null, post);
      } else {
  ...
  const popUpClose = document.querySelector(".dismiss-popup.post");
